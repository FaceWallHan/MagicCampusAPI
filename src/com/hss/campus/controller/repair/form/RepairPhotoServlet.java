...
//报修图片上传到服务器
@WebServlet(urlPatterns = "/repairPhotoServer")
public class RepairPhotoServlet extends BaseServlet {

    @Override
    public Response operatingGet(HttpServletRequest request) {
        Response<ImageResponse> response=new Response<>();
        ImageResponse imageResponse = FileUtil.receiveData(request, "repair\\");
        if (!"".equals(imageResponse.getFileName())){
            response.setCode(ResultCode.SUCCESS.code());
            response.setStatus(ResultCode.SUCCESS.status());
            response.setData(new ImageResponse(OtherUtil.IP_ADDRESS+"image/repair/"+imageResponse.getFileName()));
        }else {
